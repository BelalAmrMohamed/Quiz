export const questions = [
  {
    ...
    correct: 2,
    explanation:
      "Modifying the device-status table is a privileged instruction because it involves direct management of I/O devices. Reading the clock and issuing traps are generally non-privileged, while register computation is purely a user-mode task.",
  },
  {
    ...
    correct: 0,
    explanation:
      "A small bootstrap loader in ROM/firmware often loads a more complex 'boot block' from a fixed disk location, which then loads the full OS. This overcomes size limitations in ROM.",
  },
  {
    ...
    correct: 1,
    explanation:
      "Microkernels improve reliability by moving services to user space, but they suffer from performance overhead because communication between these services requires system calls and message passing.",
  },
  {
    ...
    correct: 3,
    explanation:
      "The stack contains temporary data such as function parameters, return addresses, and local variables. The heap is for dynamic allocation, data section for global variables, and text for code.",
  },
  {
    ...
    correct: 1,
    explanation:
      "A waiting process cannot jump directly to running; it must first enter the ready queue after its wait condition (like I/O completion) is satisfied.",
  },
  {
    ...
    options: ["Orphan", "Zombie", "Cascaded", "Preempted"],
    correct: 1,
    explanation:
      "A process that has terminated but whose parent has not yet called wait() is known as a 'zombie.' An orphan is a process whose parent terminated without calling wait().",
  },
  {
    ...
    correct: 1,
    explanation:
      "Exactly once semantics ensure the procedure executes exactly once by using acknowledgments and timestamps/counters to filter out duplicate requests caused by network retries.",
  },
  {
    ...
    correct: 0,
    explanation:
      "In the many-to-one model, many user threads map to a single kernel thread. If one user thread blocks, the entire kernel thread (and thus the process) is blocked.",
  },
  {
    ...
    options: ["1.5x", "2.5x", "Infinite speedup", "0.6x"],
    correct: 1,
    explanation:
      "Amdahl’s Law states speedup approaches 1 / S as N approaches infinity. Here, S (serial portion) is 0.4, so 1 / 0.4 = 2.5.",
  },
  {
    ...
    correct: 1,
    explanation:
      "Local variables only exist during a single function call. TLS is unique to each thread but persists across different function calls within that thread.",
  },
  {
    ...
    correct: 2,
    explanation:
      "The convoy effect occurs in FCFS when several short processes wait for one long CPU-bound process, leading to poor device and CPU utilization.",
  },
  {
    ...
    correct: 1,
    explanation:
      "If α = 1, the formula τn+1 = αtn + (1-α)τn reduces to τn+1 = tn, meaning the system only considers the actual last burst.",
  },
  {
    ...
    correct: 2,
    explanation:
      "In time-sharing (interactive) systems, minimizing response time—the time from request submission to the start of the first response—is critical for usability.",
  },
  {
    ...
    correct: 0,
    explanation:
      "Aging gradually increases the priority of processes that wait for a long time, preventing low-priority processes from starving indefinitely.",
  },
  {
    ...
    correct: 2,
    explanation:
      "A program is a passive entity (file on disk). It becomes an active process only when it is loaded into memory for execution.",
  },
];
