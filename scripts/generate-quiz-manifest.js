  ...
  async function scanDir(dir, depth = 0, metadata = {}) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // Handle exam files (only if we're inside a course). Prefer .json over .js.
        if (depth >= 3 && metadata.currentKey) {
          ...
          console.log(`            📝 Quiz: ${title} (ID: ${examId})`);
          ...
          console.warn(
            `WARNING: Quiz file "${entry.name}" found at depth ${depth} (outside course structure), skipping`,
          );
        }
      }
    }
...
async function generate() {
  ...
  const categoryTree = await buildCategoryTree(examsDir, repoRoot);
  ...
  // Write JSON manifest (primary)
  const outFileJson = path.join(outputDir, "quiz-manifest.json");
  const payload = { examList: allExams, categoryTree };
  await fs.writeFile(outFileJson, JSON.stringify(payload, null, 2), "utf8");
