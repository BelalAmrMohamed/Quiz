...
- [Quiz](#quiz)
  - [Table of contents](#table-of-contents)
    - [Complete Project Context](#complete-project-context)
...
**Project Overview**
Quiz Master is a Progressive Web App (PWA) built for college students (Computers & AI). It offers interactive quizzes, study tracking, and a gamified experience. It is designed to be hosted as a static site (e.g., GitHub Pages) and works offline.

**Technical Stack**

- **Frontend:** Vanilla JavaScript (ES Modules), HTML5, CSS3.
- **Data:** No backend. All user data (points, badges, streaks, bookmarks, and quiz history) is stored in `LocalStorage`.
- **Quiz System:** Quizzes are external JS modules stored in `/Exams`. A tool-generated `examManifest.js` acts as the central registry and category tree.
- **PWA:** Includes a service worker for offline access and installation logic.

**Key Features & Logic**

- **Game Engine:** Manages a profile system with streaks, levels, and badges. It includes a mock leaderboard to simulate competition.
- **Quiz Experience:** Supports multiple modes (timed vs. exam). Distinguishes between Multiple Choice and Essay questions. Includes a "save state" feature to prevent progress loss.
- **Summary & Export:** After a quiz, users can review answers and export results as PDF (via `html2pdf`), Markdown, or HTML.
- **Dashboard:** A hub for viewing accuracy rates, perfect scores, bookmarked questions, and detailed attempt history.

**Coding Patterns**

- **State Management:** Uses a central `gameEngine` object to get/set data.
- **Performance:** Uses DOM caching and debouncing for UI updates (like navigation and saving).
- **Modularity:** Logic is separated into functional scripts (e.g., `theme-controller.js`, `pwa-init.js`).

---

### **Brief Context**

**Project:** Quiz Master (PWA for students).
**Stack:** Vanilla JS (ES Modules), HTML/CSS, LocalStorage.
**Structure:** \* `/Exams/`: Quiz data files.

- `examManifest.js`: Registry for all quizzes.
- `gameEngine.js`: Core logic for points, streaks, and badges.
- `dashboard.js` & `summary.js`: User stats and result exports (PDF/MD).
  **Key Patterns:** \* No backend (Local storage only).
- PWA/Offline support.
- Supports MCQ and Essay formats.
- Modular, event-driven UI updates.
